...
async function waitUntilElementIsVisible(driver, element, time = DEFAULT_TIME_WAITING) {
...
async function waitForElement(driver, xpath, time = DEFAULT_TIME_WAITING) {
  ...
  await waitUntilElementIsVisible(driver, driver.findElement(By.xpath(xpath), time));
}

async function waitForElementWithClass(driver, className, time = DEFAULT_TIME_WAITING) {
  await driver.wait(until.elementLocated(By.className(className)), time * 1000);
  await waitUntilElementIsVisible(driver, driver.findElement(By.className(className), time));
}
...
async function waitForElementWithName(driver, name, time = DEFAULT_TIME_WAITING) {
  await waitUntilElementIsVisible(driver, driver.findElement(By.name(name)), time);
}
...
async function waitForElementStalenessOf(driver, element, time = DEFAULT_TIME_WAITING) {
  try {
